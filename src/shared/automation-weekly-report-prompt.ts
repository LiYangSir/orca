...
export function buildWeeklyReportPrompt(
  ...
  lines.push(
    ...
    '- 工作：新特性需说明业务背景、要解决的问题和本周动作；Bug 需说明问题现象及影响。',
    '- 进展：新特性需说明当前阶段、可见结果和产品价值；Bug 需说明已确认的原因、修复及验证结果，证据没有给出原因时写“原因暂无可确认信息”。',
    '- MR：只写 MR 标题/编号、已合并或未合并；有链接则保留。',
    '- 发布：只写已发布、未发布或状态未知。',
    '- 需关注：只有确实存在阻塞、风险或需要决策时才写，否则整行省略。',
    '篇幅默认保持简洁，通常每个产品约五行、每项一至两句话；为讲清背景、影响、原因或结果时可以适当展开，不设硬性行数或句数限制。相同产品的多个工作记录必须合并表达，不要重复。',
    '可以用主管能理解的语言解释必要原因，但不要写分支名、工作区名、commit、文件、实现过程、CI/审批详情、CR 原始状态码或证据采集方式。',

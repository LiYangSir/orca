...
describe('automation weekly report evidence', () => {
  ...
  it('keeps provider review links while neutralizing evidence instructions', () => {
    ...
    expect(prompt).toContain('新特性需说明业务背景、要解决的问题和本周动作')
    expect(prompt).toContain('Bug 需说明已确认的原因、修复及验证结果')
    expect(prompt).toContain('原因暂无可确认信息')
    expect(prompt).toContain('通常每个产品约五行、每项一至两句话')
    expect(prompt).toContain('不设硬性行数或句数限制')
